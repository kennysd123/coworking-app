...
Permitir a clientes autenticados suscribirse vía WebSocket a actualizaciones de
disponibilidad de salas en tiempo real. El gateway emite eventos `booking.created`
y `booking.cancelled` cuando el estado de una sala cambia, sin necesidad de polling.
...
El gateway SHALL rechazar conexiones sin JWT válido utilizando un middleware de Socket.IO registrado en `afterInit()` que verifica el token en `handshake.auth.token` antes de permitir la conexión.
