...
Controlar el acceso a endpoints de la API por rol de usuario mediante `JwtAuthGuard`
(autenticación) y `RolesGuard` + decorador `@Roles()` (autorización). El guard
`@Public()` excluye rutas del flujo de autenticación.
